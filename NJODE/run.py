...
def parallel_training(params=None, model_ids=None, nb_jobs=1, first_id=None,
    ...
    :param params: a list of param_dicts, each dict corresponding to one model --> THE INPUTS TO THE TRAIN FUNCTION:

        E.g. from ONJODE_train.py:
    ...
    if not os.path.exists(model_overview_file_name):
        #df_overview = pd.DataFrame(None, columns=['id', 'description'])
