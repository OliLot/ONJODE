"""
author: (NJODE) Florian Krach (ONJODE Extension) Oliver Löthgren
...
# ADDED: Computes the loss at each time, using all batches observed at that time, to be aggregated over time (and thereby over batches as well)
def compute_operator_loss(X_obs, Y_obs, Y_obs_bj, n_obs_ot, n_space_ot, batch_size, eps=1e-10,
        weight=0.5, M_obs=None, dim_to=None):
    """
    Similar to compute_loss_2_1, but now in the operator setting, with inputs
    :param X_obs: torch.tensor, the observations X_ij^b at observation time t_i. Dimension [nr_obs_Xij_overbatches_at_ti, dimension]
    :param Y_obs: torch.tensor, the observed (true) values at time ti, at each xi_ji, for all batches observed at ti. Dimension [nr_obs_Xij_overbatches_at_ti, dimension]
    :param Y_obs_bj: torch.tensor, the predicted values before a jump (ode&readout) at time ti, at each xi_ji, for all batches observed at ti. Dimension [nr_obs_Xij_overbatches_at_ti, dimension]
    :param n_obs_ot: torch.tensor, the number of additional observations over time (n) for each batch_path. Dimension [batches_at_t, ]
                in the ONJODE paper, this is n-1 => must add 1 to get total number of obs for normalization
    :param n_space_ot: torch.tensor, the number of space points J_i at each observation time ti, for all batch_paths observed at ti. Dimension [batches_at_t, ]
    :param batch_size: int or float, the total number of data batches considered in training (of which some batch_paths with obs at ti are considered here)
    """
    if M_obs is None:
        M_obs = 1.
    
    # X & Y in the forward pass are input with dimensions [batch_nr_obs_Xij, dim], only passed in on a per obs time basis
    # aggregate over output dimension
    inner_dim = 2*weight * torch.sum(M_obs * (X_obs - Y_obs) ** 2, dim=1, dtype=torch.float32) +\
            2*(1-weight) * torch.sum(M_obs * (Y_obs_bj - X_obs) ** 2, dim=1, dtype=torch.float32) #--> sum over dim --> [batch_nr_obs_Xij] 

    # aggregate over space points of each batch path at each time
    nr_groups = len(n_space_ot)
    group_ids = torch.cat([torch.full(size=(n_space_ot[i].int(),), fill_value=i, dtype=torch.long) for i in range(nr_groups)]) # 00001112222 ids for each batch's space points
    group_sums = torch.zeros(nr_groups, device=inner_dim.device)
    inner_space = group_sums.index_add_(dim=0, index=group_ids.to(inner_dim.device), source=inner_dim) # group ids relate to index in group_sums, and sums all values of the same ids from inner_dim
    
    inner_space = inner_space / n_space_ot

    # finally sum over the batch, dividing by appropriate batch ns
    outer = torch.sum(inner_space / (n_obs_ot+1), dtype=torch.float32) 

    return outer / batch_size # ultimately this calculates a batch loss. Summing all batch losses and dividing by total batch size returns the empirical loss
...
LOSS_FUN_DICT = {
    ...
    'easy': compute_loss_2,
    'very_easy': compute_loss_2_1, # io loss
    'IO': compute_loss_2_1,
    'noisy_obs': compute_loss_noisy_obs,
    'abs': compute_loss_3,
    'jump': compute_jump_loss,
    'operator': compute_operator_loss, # ADDED: operator loss function
